...
# ---- 模型路径 ----
CKPT_PATH = rf"E:\LDCT\experiments\{EXP_NAME}\best.pth"
...
# ---- 指标空间 ----
# "norm"：在 [0,1] 归一化空间算
# "hu"  ：先反归一化回 HU 再算 RMSE/MAE/NRMSE/HFEN；PSNR/SSIM 默认仍在 norm 空间
METRIC_SPACE = "hu"
...
Z_TOL_MM = 1e-2  # 与你原始 eval 脚本保持一致
...
@torch.no_grad()
def predict_full_slice_raw(model: nn.Module, qd_norm: np.ndarray, patch_size: int, device: str, batch_size: int = 32):
    """
    qd_norm: (H, W), 已经裁剪并归一化到 [0,1]
    直接按 64x64 patch 做 dense tiling inference
