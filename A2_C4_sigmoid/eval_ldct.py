...
# ---- 实验命名（和 train 保持一致）----
EXP_NAME = "unet_A2_sigmoid_l1only_1mm_train1mm_val1mm"
SEED = 0
...
# ---- 预测后处理 ----
# sigmoid 输出理论上已在 [0,1]，这里保留 clip 以统一流程
...
def predict_full_slice_raw(model: nn.Module, qd_norm: np.ndarray, patch_size: int, device: str, batch_size: int = 8):
    ...
    对 sigmoid 模型来说，这里的 raw 其实也已经是 [0,1] 范围内输出
    """
...
def build_model_and_load_ckpt(ckpt_path: str, device: str):
    model = UNet(in_channels=1, out_channels=1, base_ch=64, use_sigmoid=True).to(device)
