...
from redcnn_model import REDCNN
...
# ---- 实验命名（和 train 保持一致）----
EXP_NAME = "redcnn_1mm_l1"
SEED = 42  # 如果你的训练不是多 seed，这里只作为记录用途
...
# ---- 模型路径 ----
CKPT_PATH = rf"E:\LDCT\experiments\{EXP_NAME}\best_redcnn.pth"
...
# ---- 指标空间 ----
# "norm"：在 [0,1] 归一化空间算
# "hu"  ：先反归一化回 HU 再算 RMSE/MAE/NRMSE/HFEN；PSNR/SSIM 默认在 norm 空间
METRIC_SPACE = "hu"

# ---- 预测后处理 ----
# RED-CNN 为无 sigmoid 回归输出，因此评估前建议 clip
    ...
    for patient_id in tqdm(patient_ids, desc="External inference", ncols=120):
        ...
        for i, ds in enumerate(qd_series):
            ...
            pred_norm_raw = predict_full_slice_raw(
                model, qd_norm, PATCH_SIZE, device, batch_size=INFER_BATCH_SIZE
            )
    ...
    if isinstance(ckpt, dict):
        if "model_state_dict" in ckpt:
            ...
            try:
                ...
                raise RuntimeError(
                    f"无法从 checkpoint 中找到可用权重键。可用 keys: {list(ckpt.keys())}"
                ) from e
    else:
        model.load_state_dict(ckpt)

    model.eval()
    print(f"[INFO] RED-CNN checkpoint loaded from: {ckpt_path}")
    return model
...
def main():
    ...
    slice_json = result_dir / f"slice_metrics_{EXP_NAME}.json"
    with open(slice_json, "w", encoding="utf-8") as f:
        json.dump(all_slice_records, f, indent=2, ensure_ascii=False)

    patient_json = result_dir / f"patient_metrics_{EXP_NAME}.json"
    ...
    summary_json = result_dir / f"summary_{EXP_NAME}.json"
