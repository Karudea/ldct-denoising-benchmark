...
# ===== U-Net（Deep Supervision + No Sigmoid + Residual）=====
class UNet(nn.Module):
    ...
    - residual 的 identity 仅取输入主图像通道 x[:, 0:1, :, :]
    - forward 返回:
    ...
    def __init__(self, in_channels=1, out_channels=1, base_ch=64, deep_supervision=True):
        ...
        # Final output head（预测 residual）
        self.out_conv = nn.Conv2d(base_ch, out_channels, kernel_size=1)

        # Deep supervision heads（也预测 residual）
    ...
    def forward(self, x):
        ...
        # residual identity: 只取图像主通道，不取 thickness map
        identity = x[:, 0:1, :, :]
        ...
        # ===== main output =====
        main_residual = self.out_conv(d1)
        out = identity + main_residual

        if not self.deep_supervision:
            return out

        # ===== aux outputs =====
        aux_d2_residual = self.ds_head_d2(d2)
        aux_d3_residual = self.ds_head_d3(d3)

        identity_d2 = F.interpolate(identity, size=aux_d2_residual.shape[-2:], mode="bilinear", align_corners=False)
        identity_d3 = F.interpolate(identity, size=aux_d3_residual.shape[-2:], mode="bilinear", align_corners=False)

        out_d2 = identity_d2 + aux_d2_residual
        out_d3 = identity_d3 + aux_d3_residual
