...
class LDCTPatchNPYDataset(Dataset):
    ...
    def _load_input_tensor(self, input_path: Path) -> torch.Tensor:
        ...
        if inp.ndim == 2:
            ...
            raise ValueError(f"[ERROR] 不支持的输入维度: {inp.shape}, file={input_path}")

        if not self.cond_thickness:
            return inp

        # 路径: root / split / th / inputs / xxx.npy
    ...
    def __getitem__(self, idx):
        ...
        if tgt.ndim == 2:
            ...
            raise ValueError(f"[ERROR] 不支持的 target 维度: {tgt.shape}, file={self.target_files[idx]}")
