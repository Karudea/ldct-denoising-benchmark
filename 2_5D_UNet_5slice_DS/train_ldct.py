...
def train_one_epoch(
    ...
    for qd, fd in tqdm(loader, desc="Training", ncols=120):
        ...
        with autocast_context(device):
            ...
            if deep_supervision:
                main_out, aux_d2_out, aux_d3_out = outs

                loss_main = l1_fn(main_out, fd)
                loss_aux_d2 = l1_fn(aux_d2_out, fd)
                loss_aux_d3 = l1_fn(aux_d3_out, fd)

                loss = (
                    ds_weights["main"] * loss_main
                    + ds_weights["aux_d2"] * loss_aux_d2
                    + ds_weights["aux_d3"] * loss_aux_d3
                )
            else:
                out = outs
                loss = l1_fn(out, fd)
...
@torch.no_grad()
def validate(model, loader, l1_fn, device, deep_supervision):
    model.eval()
    running_l1 = 0.0
    running_psnr = 0.0
            ...
            json.dump(
                {
                    ...
                    "ds_weights": DS_WEIGHTS if DEEP_SUPERVISION else None,
                    "seed": seed,
